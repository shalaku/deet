...
return new class extends Migration
{
    ...
    {
        Schema::create('cast_lists', function (Blueprint $table) {
            $table->id();
            $table->foreignId('user_id')->references('id')->on('user_lists')->onDelete('cascade');
            $table->string('name_ja', 64);
            $table->string('name_kana', 64)->nullable();
            $table->string('city', 64)->nullable();
            $table->foreignId('introducer_id')->nullable()->references('id')->on('cast_lists')->onDelete('cascade');
            $table->foreignId('person_in_charge_id')->references('id')->on('user_lists')->onDelete('cascade');
            $table->string('registered_date')->nullable();
            $table->foreignId('status_id')->references('id')->on('status_lists')->onDelete('cascade');
            $table->string('rank');
            ...
            $table->foreignId('category_id')->nullable()->references('id')->on('cast_category_lists')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('cast_lists');
    }
};
