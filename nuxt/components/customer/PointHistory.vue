<script setup>
import { formatDateJpShort, formatNumber, getOrderType, getStripeStatus } from '@/libs/utilities';
    ...
    <div class="card card-dark">
      <div class="card-body fs-4">
        <div class="row">
          ...
          <div class="col-2">日時</div>
          <div class="col-2">詳細</div>
        </div>
        <div class="row" v-for="point in pointHistory" :key="point.id">
          <!-- {{point}} -->
          <div class="col-2">{{ point.matching_order?.id }} <br> {{ point.matching_order?.shown_id }} </div>
          ...
          <div class="col-2">{{ formatNumber(point.pay_amount) }}円</div>
          <div class="col-2">{{ formatDateJpShort(point.created_at) }}</div>
          <div class="col-2">
            <div v-if="point.matching_order">
              <CButton color="primary" @click="showModal(point.matching_order)">詳細</CButton>
            </div>
            <div v-else>
              <span class="fs-5">{{ getStripeStatus(point.payment_status) }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-6 mt-3">
            <div class="fs-5 fw-bold"><span class="icon-periscope d-inline-block me-2"></span>合流場所
            </div>
            <div class="fs-4">{{ shownOrderData.place_street }}</div>
          </div>

          <div class="col-3 mt-3">
            <div class="fs-5 fw-bold"><span class="icon-user d-inline-block me-2"></span>人数</div>
            <div class="fs-4">{{ shownOrderData.number_of_people }}人</div>
          </div>
          
          <div class="col-3 text-end order-badge" :class="shownOrderData.order_type">
						<span class="rank-tag fs-4 me-0">
							{{ getOrderType(shownOrderData.order_type) }}
						</span>
					</div>
          
          <div class="col-6 mt-3">
            <div class="fs-5 fw-bold">開始時間</div>
            <div class="fs-4">{{ formatDateJpShort(shownOrderData.start_date_time) }}</div>
          </div>
          
          <div class="col-6 mt-3">
            <div class="fs-5 fw-bold">終了時間</div>
            <div class="fs-4">{{ formatDateJpShort(shownOrderData.end_date_time) }}</div>
          </div>

          <div class="col-12 fs-5 text-end">ID : {{ shownOrderData.id }}  {{ shownOrderData.shown_id }}</div>
        </div>
