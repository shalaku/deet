<script setup>
import { formatDateJpShort, formatNumber, getRankPointValue } from '@/libs/utilities';
...
const shownCastData = ref([]);
const getCastData = (castId) => {
	const cast = props.requestData.castData.find(c => c.castId === castId);
    return cast ? cast.data : null; 
;}
...
if(props.requestData.details){
	props.requestData.details.forEach(detail => {
...
<!-- {{ requestData }} -->
		<div class="">
			<div class="row align-items-end justify-content-around">
				<!-- cast card -->
				<div class="row">
					<div class="col-6 mt-3" v-if="requestData.start_date_time">
						<div class="fs-4 fw-bold">開始時間</div>
						<div class="fs-3">{{ formatDateJpShort(requestData.start_date_time) }}</div>
					</div>
					<div class="col-6 mt-3" v-if="requestData.end_date_time">
						<div class="fs-4 fw-bold">解散時間</div>
						<div class="fs-3">{{ formatDateJpShort(requestData.end_date_time) }}</div>
					</div>
				</div>
				<div class="row meeting-place mt-4">
					<!-- <div class="mb-3"> -->
					<!-- <div class="fw-bold fs-4 mb-1">ID</div> -->
					<!-- <div class="fw-normal fs-3">{{ requestData.id }}</div> -->
					<!-- </div> -->
					<!-- <div class="mb-3"> -->
					<!-- <div class="fw-bold fs-4 mb-1">集合住所</div> -->
					<!-- <div class="fw-normal fs-3">{{ requestData.place_prefecture }}{{ requestData.place_municipalitie }}{{ requestData.place_street }}</div> -->
					<!-- </div> -->
					<div class="col-12">
						<div class="fw-bold fs-4 mb-1">住所</div>
						<div class="fw-normal fs-3 mt-2">
							<span class="" style="line-height: 1.8;">東京都港区六本木4-12-12穂高ビル</span><br>
							{{ requestData.place_desc }}
						</div>
					</div>
				</div>

				<div class="row mt-5" v-for="detail in requestData.details" :key="detail.id">
<!-- {{ detail }} -->
					<div v-if="detail.status == 510 || detail.status == 512 || detail.status == 513" class="mt-5">
						<div v-if="(shownCastData = getCastData(detail.cast_id))"
							class="col-12 d-flex justify-content-center">
							<!-- {{ detail }} -->
							<div class="cast-card p-0 mb-3 " @click="openCastInfo(shownCastData)">
								<div class="cast-image">
									<img :src="shownCastData?.images[0]?.image_url || dummyImageSrc" alt="cast image"
										class="uploaded-image" />
								</div>
								<div class="cast-info">
									<div class="d-flex justify-content-between">
										<span class="fs-5">{{ new Date().getFullYear() - new
											Date(shownCastData?.cast?.birthday).getFullYear() }}歳</span>
										<span class="fs-5">{{ shownCastData?.cast?.nickname }}</span>
									</div>
									<div class="fs-5 text-end">{{ shownCastData?.cast?.city }}</div>
									<div class="fs-5 text-end fw-normal">{{ shownCastData?.cast?.basic_point_price ?
										formatNumber(shownCastData.cast.basic_point_price) : "N/A" }}P
										/ 30min</div>
								</div>
							</div>
						</div>
					...
					<div class="col-6 mt-3" v-if="detail.order_acception_status == 701">
						<div class="fs-4 fw-bold">開始時間</div>
						<div class="fs-3">{{ formatDateJpShort(detail.start_date_time) }}</div>
					</div>
					<div class="col-6 mt-3" v-if="detail.order_acception_status == 701">
						<div class="fs-4 fw-bold">終了時間</div>
						<div class="fs-3">{{ formatDateJpShort(detail.end_date_time) }}</div>
					</div>

					<div class="col-6 mt-3">
						<div class="fs-4 fw-bold">基本消費ポイント</div>
						<!-- {{requestData}} -->
						<div class="fs-3">{{ 
							formatNumber(
								parseInt(getRankPointValue(detail.rank)) * 2 *
								parseInt(requestData.requested_matching_term?.split(':')[0])
								) 
							}}P
						</div>
					</div>
					...
					<div class="col-6 mt-3">
						<div class="fs-4 fw-bold">深夜ポイント</div>
						<div class="fs-3">{{ formatNumber(requestData.mid_night_fee) }}P</div>
					</div>
					<div class="col-6 mt-3" v-if="detail.designated_point > 0">
						<div class="fs-4 fw-bold">指名ポイント</div>
						<div class="fs-3">{{ formatNumber(parseInt(detail.designated_point) * 2 *
								parseInt(requestData.requested_matching_term?.split(':')[0])) }}P</div>
					</div>
				...
				<!-- <div class="price-summary mt-4 fs-4">
					<div class="text-end fw-normal">
						<div v-if="requestData.order_type === 'request'">
							<div v-if="rankPointCalc['A'] != 0">Aクラス {{ formatNumber(rankPoint['A']) }}P / 30分 x {{
								rankPersonCalc['A'] }}人</div>
							<div v-if="rankPointCalc['S'] != 0">Sクラス {{ formatNumber(rankPoint['S']) }}P / 30分 x {{
								rankPersonCalc['S'] }}人</div>
							<div v-if="rankPointCalc['SS'] != 0">SSクラス {{ formatNumber(rankPoint['SS']) }}P / 30分 x {{
								rankPersonCalc['SS'] }}人</div>
						</div>
						<div v-else>
							{{ castData[0].appliedPoint ? formatNumber(castData[0].appliedPoint.toLocaleString()) :
							"N/A" }}P/30分
						</div>
						<hr />
						<div class="d-flex justify-content-end align-items-center fw-normal mb-2">
							<div class="me-30px">時間</div>
							<div>{{ requestData.duration }}</div>
						</div>
						<hr />
						<div class="d-flex justify-content-between align-items-center fw-normal mb-2">
							<div>小計</div>
							<div>{{ formatNumber(subTotalPerHour) }} x {{ requestData.duration }}</div>
						</div>
					</div>
					<div class="d-flex justify-content-between align-items-center fw-bold">
						<div>合計</div>
						<div>{{ formatNumber(
							subTotalPerHour * parseFloat(requestData.duration.replace(/[^0-9]/g, '')) + midNightFeeTotal
							) }}P</div>
					</div>
				</div> -->
