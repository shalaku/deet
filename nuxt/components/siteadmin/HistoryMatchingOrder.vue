...
const showOrderMemos = (order) => {
  ...
  editableAdminMemo.value = order.admin_memo;
  isMemoModalVisible.value = true; // モーダルを表示
};

const saveAdminMemo = async (id,value) => {
  // Define a mapping of form fields to original data keys
  const sendData = {
    order_id: id, // bigint(20) unsigned
    admin_memo: value, // int(11)
  };

  try {
    await orderAPI.saveAdminMemo(sendData);
    isMemoModalVisible.value = false;
    Swal.fire('Success!', '処理が成功しました', 'success');
    handleSubmit();

  } catch (error) {
    console.error('Error updating order data:', error);
    Swal.fire('Error!', '処理が失敗しました', 'error');
  }
};
...
<template>
  <div>
    <div>
      <div class="d-flex justify-content-between align-items-center content-header">
        <h4 class="fw-bold fs-3 px-3 py-3">総合検索</h4>
      </div>
      <!-- search menu section -->
      <div class="container">
            ...
            <div class="register-area">
              <form @submit.prevent="handleSubmit">
                <div class="row">
                  ...
                  <div class="mb-3 col-3">
                    <label for="orderTypeSelect" class="form-label">オーダー種類</label>
                    <select v-model="order_type_select" id="orderTypeSelect" class="form-select">
                      <option value="">選択してください</option>
                      <option value="direct">Deet</option>
                      <option value="request">コール</option>
                    </select>
                  </div>
                  <div class="mb-3 col-3">
            ...
            <table class="table table-hover fs-4">
              ...
              <tbody>
                <tr v-for="order in orderHistory.orders" :key="order.id">
                  <td class="text-center" @click="showOrderMemos(order)">{{ order.id }}<br>{{ order.shown_id }}</td>
    ...
    <CModalBody>
      ...
      <div class="container-fluid">
        <div class="row">
          <div class="col-6">
            <div>
              <CBadge class="fs-5 mb-3" :class="'status-'+selectedCase.status">{{ getStatus(selectedCase.status) }}
              </CBadge>
  ...
  <CModal 
    ...
    v-on:close="() => { isMemoModalVisible = false; }"
  >
    <CModalHeader>
    ...
    <CModalBody>
      <div class="row fs-4">
        <div class="col-12">
        <p><strong>女性会員メモ</strong> {{ selectedOrder.cast_memo }}</p>
        <p><strong>管理メモ</strong></p>
          <textarea 
            v-model="editableAdminMemo" 
            class="form-control" 
            rows="4">
          </textarea>
      </div>
    ...
    <CModalFooter>
      ...
      <div class="row">
        <div class="col-12 text-end">
      <button 
        class="btn deet-btn fs-4" 
        @click="saveAdminMemo(selectedOrder.id, editableAdminMemo)">
        保存
      </button>
      </div>
      </div>
