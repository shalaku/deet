...
import { StoreName, TUIConversationService, TUIStore, TUITranslateService } from '@tencentcloud/chat-uikit-engine';
import { TUILogin } from '@tencentcloud/tui-core';
import { framework, onMounted, ref } from './adapter-vue';
import { TUIChat, TUIContact, TUIConversation, TUIGroup, TUISearch } from './components';
import { isH5 } from './utils/env';

const currentConversationID = ref<string>('');
const currentNavbar = ref<string>('message');
const currentLanguage = ref<string>('ja');
const navbarList = [
  {
    id: 'message',
    label: 'メッセージ',
...
function initLanguage() {
  currentLanguage.value = isIntlSDKAppID(props.SDKAppID) ? 'ja' : 'en';
